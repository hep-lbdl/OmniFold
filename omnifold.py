...
def reweight(events, model, batch_size=10000):
    f = model.predict(_ensure_2d(events), batch_size=batch_size, verbose=0)
    weights = f / (1. - f)
    return np.nan_to_num(weights).reshape(-1)
...
def omnifold(theta0, theta_unknown_S, iterations, model, verbose=0):
    ...
    theta0_G = _ensure_2d(theta0[:, 0])
    theta0_S = _ensure_2d(theta0[:, 1])
    theta_unknown_S = _ensure_2d(theta_unknown_S)
